...
function status_checker($row,$type,$status_id) {

	$exists=array_search_multidim($GLOBALS['array'],'id', $type . $row['Work_id']);
	
	if ($exists !="") {
			$exists_status = $GLOBALS['array'][$exists]['Status_id'];
			$row_status = $row['Status_id'];
			
			if ($exists_status < $row_status) {
					unset($GLOBALS['array'][$exists]);
				}
			}
	...
	switch($status_id) {
		...
		case "4":
			$status_color = "red";
			$status_link = "http://" . $_SERVER["HTTP_HOST"] . "/works.html#Proofed" . $row['DIMEV_number'];
		break;
		
		case "3":
			$status_color = "chartreuse";
			$status_link = "http://" . $_SERVER["HTTP_HOST"] . "/works.html#Draft" . $row['DIMEV_number'];
		break;
						
		default:
			$status_color = "aliceblue";
			$status_link = "";
		break;
		
		}
		
		return array($status_color, $status_link);
		}
...
function status_indicator($row,$type)
{
	...
	switch ($type)
	{
		case "work":
			...
			//debug_to_console($row['Status_id'];
			$status = status_checker($row,$type,$row["Status_id"]);
			$url_color= $status[0];
			$link = $status[1];
			//debug_to_console($test);
			//debug_to_console($url_color);
			//debug_to_console($row["Status_id"]);
...
//			$witness_id = UTF_clean($row['Witness_id']);
...
//			if ($type_construct = "other_Lydgate_work83") {
//			debug_to_console('Constructed Array');
//			debug_to_console($item_array);
//			}
			
//			if ($type_construct = "other_Lydgate_work142") {
//			debug_to_console('Constructed Array');
//			debug_to_console($item_array);
//			}
			...
			if (strpos($new_type, 'work') !== false)  {
				...
				{
				//	debug_to_console($to_check . " does not exist.");
...
/*			if ($source_type . $row[$source_id] = "other_Lydgate_work83") {
			debug_to_console('Link Array');
			debug_to_console($array);
			}
			
			if ($source_type . $row[$source_id] = "other_Lydgate_work142") {
			debug_to_console('Link Array');
			debug_to_console($array);
			}
*/			
...
$people_sql = "select People.id as Person_id, People.Name, People.Oxford_DNB_number from People inner join Witness_work_role_lookup on Witness_work_role_lookup.Person_id = People.id inner join Witness_work_lookup on Witness_work_lookup.id = Witness_work_role_lookup.Witness_work_id inner join Work on Witness_work_lookup.Work_id = Work.id $where_limiter group by People.id";
...
$witness_sql = "select Witness.id as Witness_id, Witness.Shelfmark as Shelfmark from Witness inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id $where_limiter group by Witness.id";
//debug_to_console($witness_sql . "LINE 700");
...
while ($row = $witness_result->fetch_array())
{
	...
	//debug_to_console($multi_work_witness_sql . "LINE 884");
...
//	debug_to_console($other_work_witness_sql . "LINE 891");
//	$other_work_witness_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id != 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") group by Work.id having count(Work.id) < 2";	
	...
	//debug_to_console($Lydgate_work_witness_sql . "LINE 898");
...
$location_sql = "select Location.id as Location_id, Location.Location_name from Location inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by Location.id";
...
$city_sql = "select Location.City_id as City_id, City.Name as Name from City inner join Location on Location.City_id = City.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by City.id";
...
$region_sql = "select Location.Region_id as Region_id, Region.Region as Region from Region inner join Location on Location.Region_id = Region.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by Region.id";
...
$country_sql = "select Location.Country_id as Country_id, Country.Country as Country from Country inner join Location on Location.Country_id = Country.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by Country.id";
$country_result = mysqli_query($mysqli, $country_sql);
array_builder($country_result, "Country_id", "Country", "country", $country_color);

$edition_sql = "select Edition.id as Edition_id, Edition.Title as Title from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness_work_lookup.Witness_id = Witness.id inner join Edition_witness_work_lookup on Edition_witness_work_lookup.Witness_work_lookup_id = Witness_work_lookup.id inner join Edition on Edition_witness_work_lookup.Edition_id = Edition.id $where_limiter group by Edition.id";
...
$people_sql = "select Work.id as Work_id, People.id as Person_id from People inner join Witness_work_role_lookup on Witness_work_role_lookup.Person_id = People.id inner join Witness_work_lookup on Witness_work_lookup.id = Witness_work_role_lookup.Witness_work_id inner join Work on Witness_work_lookup.Work_id = Work.id $where_limiter";
$people_result = mysqli_query($mysqli, $people_sql);
...
$witness_sql = "select Work.id as Work_id, Witness.id as Witness_id from Witness inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id $where_limiter";
...
//debug_to_console($witness_sql . " LINE 740");
...
while ($row = $witness_result->fetch_array())
{
	...
	$Lydgate_work_sql_result = mysqli_query($mysqli, $Lydgate_work_sql);
	//debug_to_console($Lydgate_work_sql . " LINE 1028");
	//link_builder($Lydgate_work_sql_result, "witness", "other_Lydgate_work", "Witness_id", "Work_id");
	link_builder($Lydgate_work_sql_result,"other_Lydgate_work","person","Work_id","Person_id");	

} */

$location_sql = "select Witness.id as Witness_id, Location.id as Location_id from Location inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$location_result = mysqli_query($mysqli, $location_sql);
...
$city_sql = "select Location.id as Location_id, City.id as City_id from City inner join Location on Location.City_id = City.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$city_result = mysqli_query($mysqli, $city_sql);
//debug_to_console($city_sql);
...
$region_sql = "select Location.City_id as City_id, Location.Region_id as Region_id from Region inner join Location on Location.Region_id = Region.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$region_result = mysqli_query($mysqli, $region_sql);
//debug_to_console($region_sql);
...
$country_sql = "select Location.Country_id as Country_id, Location.Region_id as Region_id from Country inner join Location on Location.Country_id = Country.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$country_result = mysqli_query($mysqli, $country_sql);
