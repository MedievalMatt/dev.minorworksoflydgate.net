...
//echo UTF_clean(json_encode($link_array));
...
//var ttest = test.map((u,i)=>[u[i].type]);

//let unique = [...new Set(ttest)]; 
...
//        .nodeOpacity('0.9')
//        .nodeResolution('25')
        //.nodeVal(function(d,i) {return console.log(d.size + " | " + d.name)})
        //.nodeVal(function(d) {return nearest(Math.log(d.size) * 10,5,5,5) || 10})
//        .nodeVal(function(d) {return d.size})
        //.nodeRelSize(function(d) {return d.size} * 5)
        //.d3Force('charge', d3.forceManyBody().strength((function(d) {return d.size})))
        //.d3Force('charge', d3.forceRadial(1000).strength(1))
  		.nodeThreeObject(node => {

  			//var test = set(data.nodes)
  			//var numId = data.reduce(function (n, item) {
    		//	return n + (item.id == node.id);
			//}, 0);
  			//console.log(numId);
  			
  			var s = new THREE.SphereGeometry(nearest((node.size / 3),5,5,5),20,20);
  			//var s = new THREE.SphereGeometry(25,25,25);
  			var lambert = new THREE.MeshLambertMaterial({ color: node.color, transparent: true, opacity: 0.9 });
  			var mesh = new THREE.Mesh(s, lambert);
  			var t = new SpriteText(node.name);
  					//console.log(getWidthOfText(node.name,"Baskerville",6));
          			t.color = 'cornsilk';
          			t.textHeight = 6;
          			t.fontFace = "Baskerville";
          			//t.position.y = (((25 / 2) * -1) - 20);
          			t.position.y = ((nearest((node.size / 3),5,5,5) * -1) -5);
          			
     		//var test = THREE.nodeVal(function(d,i) {return d.type});
...
//		.d3Force("link", d3.forceLink(data['links']).strength(-30))   
...
//        .linkDirectionalParticleWidth(.000025)
//        .linkDirectionalParticleResolution(25)
        .enableNodeDrag(true)
        .enablePointerInteraction(true)
		.onNodeHover(node => {
	//		getWidthOfText((function(d,i) {return (d.name)}),"Baskerville",6);
          // no state change
        })
        .onNodeClick(node => {
 ...
 //   			.d3Force('collide', d3.forceCollide((function(d) {return d.size}) * 5))
//          .strength('-1000');
         // .d3Force('collide', d3.forceCollide(Graph.nodeVal()))
//          .d3Force('charge').strength((Graph.nodeVal() * -1000 ));
        ...
        Graph.d3Force('center', d3.forceCenter())
             //.d3Force('charge',-1000)
//        .d3Force('manyBody', d3.forceManyBody().strength(Graph.nodeVal() * -100 ))
//       .d3Force('collide', d3.forceCollide(Graph.nodeVal()))
       	//.d3Force('collide', d3.forceCollide(25 *))
        //.d3Force('radial', d3.forceRadial(1000)).strength(1000)
          //.d3Force('charge', d3.forceRadial(1000).strength(.9))
        	 .d3Force('manyBody', d3.forceManyBody().strength((nearest(function(d,i) {return (d.size)} / 3),5,5,5) * -100))
        	 //.d3Force('manyBody', d3.forceManyBody().strength((nearest(function(d,i) {return (d.size)} / 5),5,5,5) * .5).distanceMax(function(d,i) {return (d.size)} * 10000).distanceMin(function(d,i) {return (d.size)} * 5))

        	// .d3Force('manyBody', d3.forceManyBody().strength(25).distanceMax(25))
        	// .d3Force('collide', d3.forceCollide((nearest(function(d,i) {return (d.size)}),5,5,5) * 5))
        	// .d3Force('collide', d3.forceCollide((function(d,i) {return (d.size)} * 3)))
        	.d3Force('collide', d3.forceCollide(getWidthOfText((function(d,i) {return (d.name)}),"Baskerville","6px") / 2))
        	//.d3Force('collide', d3.forceCollide(Math.max(function(d,i) {return (d.size)},getWidthOfText(function(d,i) {return (d.name)}, "Baskerville","6px"))));
        	// .d3Force("link", d3.forceLink(data['links']).distance('300'))
        	  // To make it prettier:
  		 // .d3Force('charge', d3.forceManyBody().theta(0.5).strength(-0.5));;
