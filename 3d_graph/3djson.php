...
$people_sql = "select People.id as Person_id, People.Name as Name from People inner join Witness_work_role_lookup on Witness_work_role_lookup.Person_id = People.id inner join Witness_work_lookup on Witness_work_lookup.id = Witness_work_role_lookup.Witness_work_id inner join Work on Witness_work_lookup.Work_id = Work.id $where_limiter group by People.id";
...
mysqli_free_result($people_result);

// $work_sql = "select * from Work";

$work_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name,Status_id, People.Oxford_DNB_number from People inner join Witness_work_role_lookup on Witness_work_role_lookup.Person_id = People.id inner join Witness_work_lookup on Witness_work_lookup.id = Witness_work_role_lookup.Witness_work_id inner join Work on Witness_work_lookup.Work_id = Work.id $where_limiter group by Work.id";
$work_result = mysqli_query($mysqli, $work_sql);
//debug_to_console($work_sql);
array_builder($work_result, "Work_id", "Work", "work", $work_color);
mysqli_free_result($work_result);

// $witness_sql = "select * from Witness";

$witness_sql = "select Witness.id as Witness_id, concat(Location.Short_name, ' ' , Witness.Shelfmark) as Shelfmark from Witness inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join Location on Location.id = Witness.Location_id $where_limiter group by Witness.id";
...
while ($row = $witness_result->fetch_array())
{
	...
	$multi_work_witness_sql = " select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name, Status_id, People.Oxford_DNB_number from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Witness.id =" . $row['Witness_id'] . " and Work_id in(select Work.id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work.id in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id where Witness.id =" . $row['Witness_id'] . " AND Work_id not in (" . $_GET['string'] . ") and Person_id != 1))";
	...
	mysqli_free_result($multi_work_witness_sql_result);	

	$other_work_witness_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name, Status_id, People.Oxford_DNB_number from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id != 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work_id not in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work.id in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id where Work_id not in (" . $_GET['string'] . ") and Person_id != 1))";
...
//	$other_work_witness_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id != 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") group by Work.id having count(Work.id) < 2";	
	...
	mysqli_free_result($other_work_witness_sql_result);
	
	$Lydgate_work_witness_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name, Status_id, People.Oxford_DNB_number from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work_id not in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work.id in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id where Work_id not in (" . $_GET['string'] . ") and Person_id != 1))";
	...
	mysqli_free_result($Lydgate_work_witness_sql_result);
}
mysqli_free_result($witness_result);

// $location_sql = "select * from Location";

$location_sql = "select Location.id as Location_id, Location.Location_name as Location_name from Location inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by Location.id";
...
$city_sql = "select Location.City_id as City_id, City.Name from City inner join Location on Location.City_id = City.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by City.id";
$city_result = mysqli_query($mysqli, $city_sql);
array_builder($city_result, "City_id", "Name", "city", $city_color);
mysqli_free_result($city_result);

// $region_sql = "select * from Region";

$region_sql = "select Location.Region_id as Region_id, Region.Region from Region inner join Location on Location.Region_id = Region.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by Region.id";
...
$country_sql = "select Location.Country_id as Country_id, Country.Country from Country inner join Location on Location.Country_id = Country.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter group by Country.id";
...
$edition_sql = "select Edition.id as Edition_id, Edition.Title from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness_work_lookup.Witness_id = Witness.id inner join Edition_witness_work_lookup on Edition_witness_work_lookup.Witness_work_lookup_id = Witness_work_lookup.id inner join Edition on Edition_witness_work_lookup.Edition_id = Edition.id $where_limiter group by Edition.id";
$edition_result = mysqli_query($mysqli, $edition_sql);
array_builder($edition_result, "Edition_id", "Title", "edition", $edition_color);
mysqli_free_result($edition_result);


// links

$people_sql = "select Work.id as Work_id, People.id as Person_id, People.Oxford_DNB_number from People inner join Witness_work_role_lookup on Witness_work_role_lookup.Person_id = People.id inner join Witness_work_lookup on Witness_work_lookup.id = Witness_work_role_lookup.Witness_work_id inner join Work on Witness_work_lookup.Work_id = Work.id $where_limiter";
$people_result = mysqli_query($mysqli, $people_sql);
...
$witness_sql = "select Work.id as Work_id, Work.DIMEV_number, Witness.id as Witness_id from Witness inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id $where_limiter";
...
while ($row = $witness_result->fetch_array())
{
	...
	$multi_work_sql = " select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name, Status_id, People.Oxford_DNB_number from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Witness.id =" . $row['Witness_id'] . " and Work_id in(select Work.id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work.id in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id where Witness.id =" . $row['Witness_id'] . " AND Work_id not in (" . $_GET['string'] . ") and Person_id != 1))";
	...
	mysqli_free_result($multi_work_sql_result);
	...
	$other_work_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name, Status_id, People.Oxford_DNB_number  from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id != 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work_id not in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work.id in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id where Work_id not in (" . $_GET['string'] . ") and Person_id != 1))";
	...
	mysqli_free_result($other_work_sql_result);
	...
	$Lydgate_work_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_id, Witness_work_lookup.Beginning_URL,Witness_work_role_lookup.Person_id, People.Name as Person_name, Status_id, People.Oxford_DNB_number from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work_id not in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id inner join People on Witness_work_role_lookup.Person_id = People.id where Person_id = 1 AND Witness.id =" . $row['Witness_id'] . " and Work.id not in (" . $_GET['string'] . ") and Work.id in (select Work_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness_work_role_lookup on Witness_work_role_lookup.Witness_work_id = Witness_work_lookup.id where Work_id not in (" . $_GET['string'] . ") and Person_id != 1))";
	...
	mysqli_free_result($Lydgate_work_sql_result);
...
$location_sql = "select Location.id as Location_id, Witness.id as Witness_id from Location inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$location_result = mysqli_query($mysqli, $location_sql);
//debug_to_console($location_sql);
...
$city_sql = "select Location.City_id, Location.id as Location_id from City inner join Location on Location.City_id = City.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$city_result = mysqli_query($mysqli, $city_sql);
...
$region_sql = "select Location.Region_id, Location.City_id from Region inner join Location on Location.Region_id = Region.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$region_result = mysqli_query($mysqli, $region_sql);
...
$country_sql = "select Location.Country_id, Location.Region_id from Country inner join Location on Location.Country_id = Country.id inner join Witness on Location.id = Witness.Location_id inner join Witness_work_lookup on Witness.id = Witness_work_lookup.Witness_id inner join Work on Work.id = Witness_work_lookup.Work_id $where_limiter";
$country_result = mysqli_query($mysqli, $country_sql);
//debug_to_console($country_sql);
link_builder($country_result,"region","country","Region_id","Country_id");
mysqli_free_result($country_result);

$edition_sql = "select Work.id as Work_id, Edition.id as Edition_id from Work inner join Witness_work_lookup on Work.id = Witness_work_lookup.Work_id inner join Witness on Witness_work_lookup.Witness_id = Witness.id inner join Edition_witness_work_lookup on Edition_witness_work_lookup.Witness_work_lookup_id = Witness_work_lookup.id inner join Edition on Edition_witness_work_lookup.Edition_id = Edition.id $where_limiter";
$edition_result = mysqli_query($mysqli, $edition_sql);
