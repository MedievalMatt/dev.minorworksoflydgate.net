...
function edition_checker($row, $size_array){

    global $edition_color;
    global $url_color;
    
    
            if (!in_array_r('edition' . $row['Edition_id'], $GLOBALS['array']))
                {
                            
                            $size_location = array_search_multidim($size_array,'Edition_id',$row['Edition_id']);
                            $size = $size_array[$size_location]['Size'];
                            
                            $edition_name = UTF_clean($row['Title']);
                            array_push($GLOBALS['array'], array(
                                'id' => UTF_clean("edition" . $row['Edition_id']),
                                'name' => $edition_name,
                                'type' => UTF_clean('edition'),
                                'color' => $edition_color,
                                'size' => $size,
                                'url_color' => 'aliceblue',
                                'link' => NULL));
...
function edition_linker($row) {
    //	reset($array);
        global $multi_authors;
            ...
            $target_location = array_search_multidim($GLOBALS['array'],'id','edition' . $row['Edition_id']);
...
//	reset($array);
		...
		$source_location = array_search_multidim($GLOBALS['array'],'id',$target_type . $row['subquery_work_id']);
...
function other_work_edition_linker($row) {
...
function other_Lydgate_work_edition_linker($row) {
...
$edition_size_sql = "Select count(Edition_witness_work_lookup.id) as Size, Edition.id as Edition_id, Edition.Title as Edition_title from Work inner join Witness_work_lookup on Witness_work_lookup.Work_id = Work.id inner join Edition_witness_work_lookup on Witness_work_lookup.id = Edition_witness_work_lookup.Witness_work_lookup_id inner join Edition on Edition.id = Edition_witness_work_lookup.Edition_id Group by Edition_witness_work_lookup.Edition_id";
//echo $witness_size_sql . "<BR>";
$edition_size_result = mysqli_query($mysqli, $edition_size_sql);
$edition_size = mysqli_fetch_all($edition_size_result, MYSQLI_ASSOC);
...
mysqli_free_result($edition_size_result);
...
$data_sql = "select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness_work_lookup.Witness_id, concat(Location.Short_name, ' ' , Witness.Shelfmark) as Shelfmark, Witness_work_lookup.Beginning_URL, Witness_work_role_lookup.Person_id, People.Name as Person_name, Witness_work_lookup.Status_id, People.Oxford_DNB_number, Edition.Title, Edition.id as Edition_id, Subquery.Work as subquery_work, Subquery.DIMEV_number as subquery_dimev_number, Subquery.Work_id as subquery_work_id, Subquery.Witness_id as subquery_witness_id, Subquery.Shelfmark as subquery_shelfmark, Subquery.Beginning_URL as subquery_beginning_url, Subquery.Person_id as subquery_person_id, Subquery.Person_name as subquery_person_name, Subquery.Status_id as subquery_status_id, Subquery.Oxford_DNB_number as subquery_oxford_DNB_number from People inner join Witness_work_role_lookup on Witness_work_role_lookup.Person_id = People.id inner join Witness_work_lookup on Witness_work_lookup.id = Witness_work_role_lookup.Witness_work_id inner join Work on Witness_work_lookup.Work_id = Work.id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Location on Location.id = Witness.Location_id inner join Edition_witness_work_lookup on Witness_work_lookup.id = Edition_witness_work_lookup.Witness_work_lookup_id inner join Edition on Edition_witness_work_lookup.Edition_id = Edition.id inner join( select Work.Work, Work.DIMEV_number, Work.id as Work_id, Witness.id as Witness_id, concat(Location.Short_name, ' ' , Witness.Shelfmark) as Shelfmark, Witness_work_lookup.Beginning_URL, Witness_work_role_lookup.Person_id, People.Name as Person_name, Status_id, People.Oxford_DNB_number from People inner join Witness_work_role_lookup on Witness_work_role_lookup.Person_id = People.id inner join Witness_work_lookup on Witness_work_lookup.id = Witness_work_role_lookup.Witness_work_id inner join Work on Witness_work_lookup.Work_id = Work.id inner join Witness on Witness.id = Witness_work_lookup.Witness_id inner join Location on Location.id = Witness.Location_id" . $URL_limiter . ") as Subquery on Subquery.Witness_id = Witness.id" . $where_limiter;
...
//echo "<BR>";
    ...
    edition_checker($row, $edition_size);
}
...
//edition_linker($row);
//other_work_edition_linker($row);
//other_Lydgate_work_edition_linker($row);
}
