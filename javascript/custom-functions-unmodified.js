...
function compare_toggle_visibility(id, line, collection) {
    ...
    var f = document.getElementsByClassName('compare_item');
    for (var j=0; j < f.length; j++) {
        f[j].style.display= 'none';
    }
    
    var comparisonHeight = $(".comparison").height();
    console.log(comparisonHeight);
    
    e.style.display = ((e.style.display != 'block') ? 'block': 'none');
    for(var i = 0; i < f.length; i++) {
        if (f[i].id == line) {
            f[i].style.display= ((f[i].style.display != 'inline-block') ? 'inline-block': 'none');
        }
    }
    ...
    if (e.style.display == 'block') {
        $(".surface").height(surfaceHeight + eHeight);
    }
    else
    {$(".surface").height(surfaceHeight - eHeight);}
