...
$test = "java -Xms128m -Xmx1024m -XX:+UseCompressedOops -cp saxon9he.jar net.sf.saxon.Query -t -q:test.xq -o:$filename line=$line marginalia=$marginalia zone=$zone collection=file:/home/matrygg/minorworksoflydgate.net/XML/$collection";
echo "<script>console.log( 'Debug Objects: " . $test . "' );</script>";
$text = exec ("java -Xms128m -Xmx1024m -XX:+UseCompressedOops -cp saxon9he.jar net.sf.saxon.Query -t -q:test.xq -o:$filename line=$line marginalia=$marginalia zone=$zone collection=file:/home/matrygg/minorworksoflydgate.net/XML/$collection");
echo $text;
