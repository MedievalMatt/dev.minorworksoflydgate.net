...
let $sd := $y//tei:sourceDesc[1]/@xml:id
let $z := $s/tei:zone[@n=$zone]
let $x := $z/tei:zone[@n=$marginalia]
let $test := $x
(:let $where := if ($marginalia != "") then $x//tei:line/@n = $line else $z//tei:line/@n = $line:)
let $l := if ($marginalia != "") then $x/tei:line[@n=$line] else $z/tei:line[@n=$line]
let $side := substring-before($l/../../../@n,"e")
let $folio := translate($l/../../../../@xml:id,'.','')
...
let $g := concat($sd,"_",$folio,$side,".html")
let $o:= if($l/tei:note) then local:remove-elements($l,$remove-list) else $l

(:where ($where):)

return 

(: <item>{$w}: <ref target="{$g}" zonenum="{$zone}">{$o}</ref></item>}:)
$test}
